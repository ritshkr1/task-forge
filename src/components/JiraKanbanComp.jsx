...
const KanbanCard = ({ item }) => {
  return (
    <div className="bg-card-bg border border-border-main p-3 rounded-[3px] shadow-sm mb-2 hover:bg-card-bg-hover cursor-pointer group transition-colors">
      ...
      {item.epic && (
        <div
          className={`inline-block px-1.5 py-0.5 rounded-[3px] text-[10px] font-bold uppercase tracking-wider mb-3 border ${item.epicColor}`}
        >
          {item.epic}
        </div>
      )}
      ...
      <div className="flex items-center justify-between mt-1">
        <div className="flex items-center gap-1.5">
          ...
          {item.type === "story" ? (
            ...
            <div className="w-3.5 h-3.5 bg-blue-500 rounded-[1px] flex items-center justify-center text-white text-[8px] font-bold">
              ✓
            </div>
          )}
    ...
    const boardTasksData = boardTask.map((board) => {
      const columnData = tasks.reduce((acc, task) => {
        ...
        if (task.status === board.title) {
          acc.push(item);
        }else if(task.status === 'TODO' && board.title === 'TO DO'){
          acc.push(item)
        }
        return acc;
      },[]);
      return {...board, count:columnData.length || 0, items: columnData}
    });
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <input
              type="text"
              value={searchText}
              onChange={(e) => handleSearchBoard(e.target.value)}
          ...
          <div className="flex -space-x-1">
            <div className="w-7 h-7 rounded-full bg-blue-500 border-2 border-bg-secondary flex items-center justify-center text-[10px] text-white font-bold">
              RK
            </div>
              ...
              <div className="p-3 pb-2 flex items-center justify-between shrink-0 group mb-1">
                <div className="flex items-center gap-2 text-xs font-bold text-text-secondary uppercase tracking-wider">
                  ...
                  {column.count !== null && !column.isDoneColumn && (
                    <span className="bg-neutral-200 text-text-primary px-2 py-0.5 rounded-full min-w-[20px] text-center">
                      {column.count}
                    </span>
                  )}
                  {column.isDoneColumn && (
